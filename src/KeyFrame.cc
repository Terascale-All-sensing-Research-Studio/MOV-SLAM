...
namespace MOV_SLAM
{
    ...
    KeyFrame::KeyFrame() : mnFrameId(0), mTimeStamp(0), mnGridCols(FRAME_GRID_COLS), mnGridRows(FRAME_GRID_ROWS),
                           ...
                           NLeft(0), NRight(0), mnNumberOfOpt(0), mbHasVelocity(false), mvVF(static_cast<vector<VideoFeature>>(NULL))
    {
    }

    KeyFrame::KeyFrame(Frame &F, Map *pMap) : bImu(pMap->isImuInitialized()), mnFrameId(F.mnId), mTimeStamp(F.mTimeStamp), mnGridCols(FRAME_GRID_COLS), mnGridRows(FRAME_GRID_ROWS),
                                              ...
                                              mImuCalib(F.mImuCalib), mvpMapPoints(F.mvpMapPoints),
                                              mbFirstConnection(true), mpParent(NULL), mDistCoef(F.mDistCoef), mbNotErase(false), mnDataset(F.mnDataset),
                                              ...
                                              mvKeysRight(F.mvKeysRight), NLeft(F.Nleft), NRight(F.Nright), mTrl(F.GetRelativePoseTrl()), mnNumberOfOpt(0), mbHasVelocity(false), mvVF(F.mvVF), mImage(F.imgLeft), mDescriptors(F.mDescriptors)
    {
        mnId = nNextId++;
