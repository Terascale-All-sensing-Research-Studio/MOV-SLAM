...
int main(int argc, char **argv)
{
    ...
    {
        cerr << endl
             << "Usage: ./mono_kitti_tartanair path_to_settings path_to_stream" << endl;
        return 1;
    }

    // Create SLAM system. It initializes all system threads and gets ready to process frames.
    MOV_SLAM::System SLAM(argv[1], MOV_SLAM::System::MONOCULAR, true, 0, std::string(), true);
    ...
    while (true)
    {
        ...
        if (smv)
        {
            ...
            std::chrono::steady_clock::time_point pbegin = std::chrono::steady_clock::now();
            // Pass the image to the SLAM system
            SLAM.TrackMonocular(tframe, smv);

            std::chrono::steady_clock::time_point t2 = std::chrono::steady_clock::now();

            // Wait to load the next frame
            double duration = std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(t2 - t1).count();
            imageDur += std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(t2 - pbegin).count();
    ...
    SLAM.SaveKeyFrameTrajectoryEuRoC("TrajectoryEUROC.txt");
