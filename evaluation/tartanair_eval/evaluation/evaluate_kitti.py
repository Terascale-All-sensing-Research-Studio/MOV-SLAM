...
def calculate_ave_errors(errors,lengths=[10,20,30,40,50,60,70,80]):
    ...
    for length in lengths:
        ...
        if len(rot_error_each_length)==0:
            ...
            print('Translation Errors for each length [%]')
            print(sum(tra_error_each_length)/len(tra_error_each_length)*10)
            print('Rotation Errors for each length [deg/m]')
            print(sum(rot_error_each_length)/len(rot_error_each_length)*180/np.pi)
